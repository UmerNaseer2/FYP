...
export default async function DriftPage({
  ...
  // The tracked list backs the selector, the default selection, and the empty
  // state. Cheap metadata read (no live DB hit).
  const tracked = await listTrackedSchemas();

  // Nothing tracked → one honest empty state for both tabs.
  if (tracked.length === 0) {
    return (
      <Workspace tab={tab} selectedId={null}>
  ...
  const requested = sp.schema ? Number(sp.schema) : NaN;
  const selected = tracked.find((t) => t.id === requested) ?? byPriority[0];
  const selectedId = selected.id;

  return (
    <Workspace tab={tab} selectedId={selectedId}>
      {tab === "detail" ? (
        <DetailTab tracked={byPriority} selectedId={selectedId} />
      ) : (
        <AuditTab />
      )}
    </Workspace>
...
function Workspace({
  ...
  const detailHref = selectedId
    ? `/drift?tab=detail&schema=${selectedId}`
    : "/drift?tab=detail";
  return (
    <div className="max-w-[1100px] mx-auto px-8 py-10 space-y-7">
      ...
      <div className="flex items-center gap-1" style={{ borderBottom: "1px solid var(--border)" }}>
        <Tab href={detailHref} active={tab === "detail"}>
          Drift detail
        </Tab>
        <Tab href="/drift?tab=audit" active={tab === "audit"}>
          Audit log
        </Tab>
      </div>
