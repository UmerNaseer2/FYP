...
/** Lower-cased type name with any length/precision stripped: "varchar(30)" → "varchar". */
function baseType(typeDisplay: string): string {
  return typeDisplay.toLowerCase().split("(")[0].trim();
}
    ...
    if (table.indexes) {
      ...
      for (const [, names] of byShape) {
        ...
        advice.push({
          id: "duplicate-index",
          severity: "medium",
          title: "Two indexes with the same definition",
          object: `${table.name} (${names.join(", ")})`,
          detail:
            "Identical indexes cost twice the disk and twice the write work — " +
            "every INSERT, UPDATE and DELETE maintains both — and the planner " +
            "can only ever use one of them.",
          fix: `DROP INDEX ${quote(schema)}.${quote(names[1])};`,
        });
      }
    ...
    for (const column of table.columns) {
      ...
      if (column.columnDefault?.toLowerCase().startsWith("nextval(")) {
        advice.push({
          ...
          fix:
            `ALTER TABLE ${qualified(schema, table.name)} ` +
            `ALTER COLUMN ${quote(column.name)} DROP DEFAULT;\n` +
            `ALTER TABLE ${qualified(schema, table.name)} ` +
            `ALTER COLUMN ${quote(column.name)} ADD GENERATED BY DEFAULT AS IDENTITY;`,
        });
      }
    }
  ...
  for (const t of tables) {
    ...
    if (t.n_live_tup >= 1_000 && t.n_dead_tup > t.n_live_tup * 0.2) {
      advice.push({
        ...
        fix:
          `VACUUM (ANALYZE) ${quote(t.table_name)};\n` +
    ...
    if (t.last_analyzed === null && t.n_live_tup >= 1_000) {
      advice.push({
        ...
        fix: `ANALYZE ${quote(t.table_name)};`,
      });
    }
  ...
  for (const index of indexes) {
    ...
    advice.push({
      ...
      fix:
        ...
        `DROP INDEX ${quote(index.index_name)};`,
    });
  }
